...
test-e2e:
	...
	docker run --rm --volume `pwd`/test/e2e/cypress.json:/cypress.json --platform linux/amd64 --volume `pwd`/test/e2e/cypress:/cypress --volume `pwd`/results:/results  --network calc-test-e2e cypress/included:4.9.0 --browser chrome || true
