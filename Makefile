...
server:
	docker run --rm --volume `pwd`:/opt/calc --name apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app/api.py -p 5001:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
...
test-e2e:
	...
	docker run --platform linux/amd64 --rm \
		...
		cypress/included:4.9.0 || true
	docker rm --force apiserver
	docker rm --force calc-web
	@if [ -f results/cypress_result.xml ]; then \
		docker run --rm --volume `pwd`:/opt/calc --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest \
		junit2html results/cypress_result.xml results/cypress_result.html; \
	else \
		echo "⚠️  Archivo de resultados Cypress no encontrado. Saltando conversión a HTML."; \
	fi
